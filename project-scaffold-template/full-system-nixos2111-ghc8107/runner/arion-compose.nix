{ pkgs, ... }: {
  config.project.name = "java-analyzer-runner";
  config.services = {
    ...
    java-analyzer-runner-service = {
      ...
      service.defaultExec = [
        "sh"
        "-c"
        "${pkgs.java-analyzer-runner.java-analyzer-runner-exe.outPath}/bin/java-analyzer-runner --pageUrl https://en.swissquote.com/forex/pricing/trading-conditions --uploadUrl http://postgrest-service:3000/api/updateSQTradingCondition"
      ];
      service.ports = [ ];
      service.environment.ENV1 =
        "${pkgs.java-analyzer-runner.java-analyzer-runner-exe.outPath}/share/doc";
