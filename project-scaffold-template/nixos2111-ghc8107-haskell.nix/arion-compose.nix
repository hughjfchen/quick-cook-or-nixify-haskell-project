  ...
  config.services = {
    ...
    {{name}}-service = {
      ...
      service.defaultExec = [
        "sh"
        "-c"
        "${pkgs.{{name}}.{{name}}-exe.outPath}/bin/{{name}} --pageUrl https://en.swissquote.com/forex/pricing/trading-conditions --uploadUrl http://postgrest-service:3000/api/updateSQTradingCondition"
      ];
      service.ports = [ ];
      service.environment.ENV1 =
        "${pkgs.{{name}}.{{name}}-exe.outPath}/share/doc";
