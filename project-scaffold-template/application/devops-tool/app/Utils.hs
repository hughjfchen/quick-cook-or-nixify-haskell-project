...
oneFileInDir ::
  (WithError err m, As err AppError) =>
  [Path Abs File] ->
  m (Path Abs File)
oneFileInDir [] =
  throwError $
    as $
      NoSuchFileInDir "Found no such file with the searching criterates in dir."
oneFileInDir [h] = pure h
oneFileInDir files@(_ : _) =
  throwError $
    as $
      TooManySameFileInDir $
        unlines
          [ "Found more than one files ",
            "with the searching criteriates in dir.",
            "They are: ",
            show files
          ]
...
lastlastAfterSplit s =
  head . LN.fromList . tail . LN.reverse
    . LN.fromList
    . T.splitOn s
    . toText
...
firstfirstAfterSplit s =
  head . LN.fromList . tail . LN.fromList
    . T.splitOn s
    . toText
...
threadDumpFileNameP = do
  threadDumpName >> dumpFileNameSep >> dumpFileJVMPID >> dumpFileNameSep
    >> dumpFileDate
    >> dumpFileNameSep
    >> dumpFileTime
    >> eof
...
heapDumpFileNameP = do
  heapdumpName >> dumpFileNameSep >> dumpFileJVMPID >> dumpFileNameSep
    >> dumpFileDate
    >> dumpFileNameSep
    >> dumpFileTime
    >> eof
...
isScriptGenThreadDump f =
  toFilePath f
    & toText
    & parse threadDumpFileNameP ""
    & isRight
