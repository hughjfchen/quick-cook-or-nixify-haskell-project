    ...
    config = lib.mkIf
      ...
      (lib.attrsets.setAttrByPath [ "systemd" "services" pkgName ] {
        ...
        serviceConfig = {
          ...
          '';
          ExecStop =
            "${my-openresty-bin-sh}/bin/${my-openresty-bin-sh.name} -s stop";
          Restart = "on-failure";
        };
      });
  };
...
in rec {
  ...
  mk-my-openresty-reference =
    nPkgs.writeReferencesToFile mk-my-openresty-service-systemd-setup-or-bin-sh;
