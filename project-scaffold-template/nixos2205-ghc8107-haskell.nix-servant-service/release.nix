...
let
  ...
  {{name}}-bin-sh = nPkgs.writeShellApplication {
    ...
    text = ''
      ${sPkgs.java-analyzer-runner.java-analyzer-runner-exe.exeName} --config.file="{% raw %}${{% endraw %}{{name}}-config-kv{% raw %}}{% endraw %}" "$@"
    '';
  };
  ...
  {{name}}-service = { lib, pkgs, config, ... }: {
    ...
    config = lib.mkIf
      ...
      (lib.attrsets.setAttrByPath [ "systemd" "services" pkgName ] {
        ...
        serviceConfig = {
          Type = "forking";
          User = "{% raw %}${{% endraw %}{{name}}-env.runner.processUser{% raw %}}{% endraw %}";
          ExecStart =
            "{% raw %}${{% endraw %}{{name}}-bin-sh{% raw %}}{% endraw %}/bin/{% raw %}${{% endraw %}{{name}}-bin-sh.name{% raw %}}{% endraw %} --command=Start";
          Restart = "on-failure";
        };
      });
  };
...
in rec {
  ...
  mk-{{name}}-deploy-sh = deploy-packer.mk-deploy-sh {
    ...
    execName = "{% raw %}${{% endraw %}{{name}}-bin-sh.name{% raw %}}{% endraw %}";
